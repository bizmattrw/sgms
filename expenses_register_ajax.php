...
$searchQuery = " ";
if ($searchValue != '') {
    $searchQuery = " and (expense like '%" . $searchValue . "%') ";
}

## Total number of records without filtering
$sel = mysqli_query($con, "select count(*) as allcount from expenses_register");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of records with filtering
$sel = mysqli_query($con, "select count(*) as allcount from expenses_register WHERE 1 " . $searchQuery) or die(mysqli_error($con));
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "select * from expenses_register WHERE 1 " . $searchQuery . " order by expense asc  limit " . $row . "," . $rowperpage;
