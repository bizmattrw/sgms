...
$sel = mysqli_query($con, "select count(*) as allcount from saving");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of records with filtering
$sel = mysqli_query($con, "select count(*) as allcount from saving WHERE 1 " . $searchQuery) or die(mysqli_error($con));
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "select s.date,s.id,m.names,m.idcard,m.phone,s.saving from members m,saving s WHERE 1 " . $searchQuery . " and m.id=s.pin order by s.date desc  limit " . $row . "," . $rowperpage;
