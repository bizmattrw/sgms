...
$sel = mysqli_query($con, "select count(*) as allcount from expenses");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of records with filtering
$sel = mysqli_query($con, "select count(*) as allcount from expenses WHERE 1 " . $searchQuery) or die(mysqli_error($con));
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "select * from expenses WHERE 1 " . $searchQuery . " order by rawItem asc  limit " . $row . "," . $rowperpage;
