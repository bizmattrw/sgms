...
$sel = mysqli_query($con,"select count(*) as allcount from members");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of records with filtering
$sel = mysqli_query($con,"select count(*) as allcount from members WHERE 1 ".$searchQuery) or die(mysqli_error($con));
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "select * from members WHERE 1 ".$searchQuery." order by id desc  limit ".$row.",".$rowperpage;
