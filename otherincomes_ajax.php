...
$sel = mysqli_query($con, "select count(*) as allcount from otherincomes");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of records with filtering
$sel = mysqli_query($con, "select count(*) as allcount from otherincomes WHERE 1 " . $searchQuery) or die(mysqli_error($con));
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "select * from otherincomes WHERE 1 " . $searchQuery . " order by income asc  limit " . $row . "," . $rowperpage;
